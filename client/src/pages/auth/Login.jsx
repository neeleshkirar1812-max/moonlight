...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      addToast({ title: 'Authentication Failed', message: err.message || 'Invalid email or password.', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-lg w-full bg-[#121215]/90 border border-gold-500/30 rounded-3xl p-6 sm:p-10 shadow-2xl relative z-10 backdrop-blur-2xl animate-fade-in space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-neutral-400 max-w-sm mx-auto">
            {activeTab === 'customer' && 'Access your private high-resolution wedding albums, 4K films, download selections & receipts.'}
        ...
        {/* Registration Request Link for Couples */}
        {activeTab === 'customer' && (
          <div className="p-3.5 rounded-2xl bg-white/5 border border-gold-500/30 flex items-center justify-between">
        ...
        <form onSubmit={handleLogin} className="space-y-4 text-xs">
          ...
          >
            {loading ? 'Verifying Credentials...' : `Sign In to ${activeTab === 'customer' ? 'Couple Sanctuary' : activeTab === 'admin' ? 'Studio Command Center' : 'Crew Portal'}`}
